...
public class CMIS242ASG3Brandstetter {
	...
	public static class TemperatureConverter extends Converter {
		...
		// return Double.NaN
		double convert() {
			// We use getter method of super class to get input value and check if it has
			// default value. If yes, we return same as result, otherwise we calculate
	...
	public static class GUIConverter {
		GUIConverter() {
			...
			// GUI will have 3 buttons: "Distance Converter", "Temperature Converter", and
			// "Exit".
			JButton but1 = new JButton("Distance");
			JButton but2 = new JButton("Temperature");
			JButton but3 = new JButton("Exit");

			// We add them to the Panel, add panel to frame and set size and layout of
			// frame.
			panel.setLayout(new BorderLayout());
			panel.add(but1, BorderLayout.WEST);
			panel.add(but2, BorderLayout.EAST);
			panel.add(but3, BorderLayout.SOUTH);
			...
			// When user clicks Distance Converter, an input dialog will pop up where user
			// can type value and click OK. Once user clicks OK, message dialog will pop up
			// with converted result.
			but1.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					String str = JOptionPane.showInputDialog("Input distance in miles to convert to kilometers::"); // str will store input string
																							// from
																							// JOptionPane.showInputDialog()
					Double miles = Double.parseDouble(str);// Conversion to Double and storing in miles variable
					DistanceConverter object = new DistanceConverter(miles);// Creation of new DistanceConverter obj
																			// using miles
					JOptionPane.showMessageDialog(j, miles + " Miles equals in " + object.convert() + " Kilometers.");// Calling convert() of
																								// obj and displaying
																								// result in
			...
			// When user clicks on Temperature button, an input dialog will pop up to input
			// value and then when clicks OK, the message dialog with pop up with converted
			// result
			but2.addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					String str = JOptionPane.showInputDialog("Input Fahrenheit temp to convert to Celsius:"); // str will store
																									// input string from
																									// JOptionPane.showInputDialog()
					Double far = Double.parseDouble(str);// Conversion to Double and storing in far variable
					TemperatureConverter object = new TemperatureConverter(far);// Creation of new TemperatureConverter
																				// obj using far
					JOptionPane.showMessageDialog(j, far + " F equals in " + object.convert ()+ " C");
				}
			});
